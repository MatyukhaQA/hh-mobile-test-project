...
def test_job_search():
    ...
    with allure.step('Проверяем страницу вакансии'):
...
def test_login():
    ...
    with allure.step('Проверяем профиль'):
...
def test_information_about_app():
    with allure.step('Открываем страницу "O приложении"'):
        browser.element(
    ...
    with allure.step('Проверяем страницу'):
        ...
        browser.element((AppiumBy.XPATH, '//android.widget.TextView[@text="Версия: 7.2"]'))\
            .should(have.text('Версия: 7.2'))
...
def test_main_page():
    with allure.step('Открываем главную страницу'):
        browser.element((AppiumBy.ID, 'ru.hh.android:id/fragment_intentions_onboarding_choose_direction_image_close'))\
            .click()
    with allure.step('Проверяем главную страницу'):
...
def test_view_vacancy():
    with allure.step('Открываем вакансию'):
        browser.element((AppiumBy.ID, 'ru.hh.android:id/fragment_intentions_onboarding_choose_direction_image_close'))\
            .click()
